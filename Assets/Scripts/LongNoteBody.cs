...
public class LongNoteBody : MonoBehaviour {
    ...
    void Update() {
        ...
        // game is right Å® left, so X axis is travel direction
        float left = Mathf.Min(hp.x, tp.x);
        float right = Mathf.Max(hp.x, tp.x);

        // if still active, clip the part already passed the judge line
        if (judge != null && !isBroken) {
            float jx = judge.position.x;
            left = Mathf.Max(left, jx);
            if (right <= left + 0.001f) {
                // body fully consumed at judge line
                Destroy(gameObject);
                return;
            }
        }

        // center position between visible left/right
